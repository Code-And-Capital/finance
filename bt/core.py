...
class SecurityBase(Node):
    ...
    def update(self, date, data=None, inow=None):
        ...
        if np.isnan(self._price):
            if is_zero(self._position):
                ...
                raise Exception(
                    "Position is open (non-zero: %s) and latest price is NaN for security %s on %s. Cannot update node value."
    ...
    def allocate(self, amount, update=True):
        ...
        if is_zero(self._price) or np.isnan(self._price):
            raise Exception(
                "Cannot allocate capital to %s because price is %s as of %s"
    ...
    def transact(self, q, update=True, update_self=True, price=None):
        ...
        if price is not None and not self._bidoffer_set:
            raise ValueError(
                'Cannot transact at custom prices when "bidoffer" has not been passed during setup to enable bid-offer tracking.'
            )
    ...
    def update(self, date, data=None, inow=None):
        ...
        if np.isnan(coupon):
            if is_zero(self._position):
                ...
                raise Exception(
                    "Position is open (non-zero) and latest coupon is NaN for security %s on %s. Cannot update node value."
                    % (self.name, date)
                )
        else:
